...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'employees') {
      ...
      if (!employeeSheet) {
        return NextResponse.json({ error: 'Employees sheet not found' }, { status: 404 })
      }
      ...
      if (!storeSheet) {
        return NextResponse.json({ error: 'Stores sheet not found' }, { status: 404 })
      }
    ...
    return NextResponse.json({ error: 'Failed to fetch data from sheets' }, { status: 500 })
  }
}